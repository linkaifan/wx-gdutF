...
const getCETProofUrl = () => {
    return new Promise(async (resolve) => {
        const { url, cookie, ua } = (await request({ url: '/cet/getYzm', method: 'GET' })).data;
        CETCookie = cookie;
        CETUA = ua;
        resolve(url);
    });
...
const getCETScore = (name, id) => {
    return new Promise(async (resolve, reject) => {
        const res = await request({ url: '/cet', data: { cookie: CETCookie, ua: CETUA, xm: name, zkzh: id } });
        if (res.code === 200 && res.data.cet !== '') {
            const { data } = res;
            resolve([{
                subject: 'CET',
                score: data.cet || 'CET',
...
//18.09.13 f改，将getToken去掉一层promise封装，可以把request的reject情况暴露去app.vue中
